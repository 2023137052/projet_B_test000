...
public class Player
{
    private int hp = 100;                                       //변수 Hp를 private하게 선언 100임력
    private int Power = 50;                                     //변수 Power를 private하게 선언 50입력
...
public class Test_008 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        playerHp.text ="Player2 HP : "+ mPlayer2.GetHp().ToString();                        //Player의 Hp를 UI로 표시

        if (Input.GetMouseButtonDown(0))                                //왼쪽 마우스
        {
            mPlayer.Damage(1);                                          //Player 메소드 Damage 호출
        }
        if (Input.GetMouseButtonDown(0))                                //오른쪽 마우스
        {
